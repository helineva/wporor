...
describe "BeermappingApi" do
  ...
  it "When HTTP GET returns no entries, an empty array is returned" do
    ...
    stub_request(:get, /.*sepoo/).to_return(body: canned_answer, headers: { 'Content-Type' => "text/xml" })

    places = BeermappingApi.places_in("sepoo")
  ...
  it "When HTTP GET returns many entries, all of them are parsed and returned" do
    ...
    expect(places[0].street).to eq("Kaarnatie 20")
    expect(places[1].name).to eq("Sonnisaari")
    expect(places[1].street).to eq("Tuotekuja 1")
  end
end
